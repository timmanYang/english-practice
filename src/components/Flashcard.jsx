...
export default function Flashcard() {
  ...
  const [localShuffled, setLocalShuffled] = useState(() =>
    pickItems(list, list.length, `flashcard_${grade}_${isSentence ? 'sentence' : 'word'}`)
  )
  ...
  const shuffle = useCallback(() => {
    const reshuffled = pickItems(list, list.length, `flashcard_${grade}_${isSentence ? 'sentence' : 'word'}`)
    setIndex(0)
    setShowZh(false)
        ...
        >
          {!showZh ? (
            <motion.div
              ...
              animate={{ rotateY: 0 }}
            >
              <span className="hint">{isSentence ? '🇬🇧 英文句式' : '🇬🇧 英文'}</span>
